    ...
    <main class=" overflow-y-scroll overflow-hidden grow  h-full relative " style="contain:content">
        ...
        <ul class="p-2 grid w-full spacey-y-2">
            ...
            <li
             ...
             class="py-3 hover:bg-pink-50 rounded-2xl dark:hover:bg-pink-900/70 transition-colors duration-150 flex gap-4 relative w-full cursor-pointer px-2 {{$conversation->id==$selectedConversation?->id ? 'bg-pink-300':''}}">
                        ...
                        <x-dropdown align="right" width="48">
                            ...
                            <x-slot name="content">
                                ...
                                <div class="w-full p-1">
                                    ...
                                    class="items-center gap-3 flex w-full px-4 py-2 text-left text-sm leading-5 text-gray-500 hover:bg-pink-300 transition-all duration-150 ease-in-out focus:outline-none focus:bg-gray-100">
...
<script>
    ...
    // Get all elements with the 'avatar' class
    const avatarElements = document.querySelectorAll('.avatar');

    // Iterate over each avatar element and set its src attribute to a random avatar image
    avatarElements.forEach((avatarImg) => {
        // Get a random index from the avatars array
        const randomIndex = Math.floor(Math.random() * avatars.length);
        // Set the src attribute of the img element to a random avatar image
